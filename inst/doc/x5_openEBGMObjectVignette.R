...
library(openEBGM)
data(caers)
proc <- processRaw(data = caers, stratify = FALSE, zeroes = FALSE)
squashed <- squashData(data = proc)
theta_init <- data.frame(alpha1 = c(0.2, 0.1, 0.3, 0.5, 0.2),
                         beta1  = c(0.1, 0.1, 0.5, 0.3, 0.2),
                         alpha2 = c(2,   10,  6,   12,  5),
                         beta2  = c(4,   10,  6,   12,  5),
                         p      = c(1/3, 0.2, 0.5, 0.8, 0.4))
hyper_estimate <- autoHyper(data = squashed, theta_init = theta_init, 
                            zeroes = FALSE, squashed = TRUE, N_star = 1)

## ------------------------------------------------------------------------
ebout <- ebScores(processed = proc, hyper_estimate = hyper_estimate,
                  quantiles = c(5, 95)) #For the 5th and 95th percentiles
ebout_noquant <- ebScores(processed = proc, hyper_estimate = hyper_estimate,
